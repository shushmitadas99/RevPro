...
def test_get_all_users(mocker):
    ...
    def mock_get_all_users(self):
        # return predefined list of user objects as we don't test the dependency.
...
def test_get_user_by_id_positive(mocker):
    ...
    def mock_get_user_by_id(self, user_id):
        if user_id == "1":
            ...
            return None
    # Mocking the real ger_user_by_id in the dao layer with the fake one defined above
    mocker.patch('dao.user_dao.UserDao.get_user_by_id', mock_get_user_by_id) # we don't add parentheses because
    # we are not calling the function. We are only passing it as an argument.
...
def test_delete_user_by_id_positive(mocker):
    ...
    def mock_delete_user_by_id(self, user_id):
        if user_id == "1":
            ...
            return False
    # UserDao.delete_user_by_id returns a boolean value in this case
    ...
    # Assert
    assert actual is None  # since the UserService.delete_user_by_id has no return value
