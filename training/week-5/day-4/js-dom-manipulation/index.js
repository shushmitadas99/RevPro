console.log(document); //shows the dom (html elements) tree on the Console
...
console.log(h1_element.innerHTML);  //innerHTML prints out the contents within the specified element
...
getPokemonBtn.addEventListener('click', () => {
    fetch(`https://pokeapi.co/api/v2/pokemon/${pokemonIdInput.value}`, { //fetch particular endpoint => fetch returns a promise
        'method': 'GET'
    }).then((res) => { //when the promise is resolved, we get a response
        return res.json(); //we convert that response to json and that response returns yet another promise
    }).then((data) => { // this 2nd promise gives you the data
        let tRow = document.createElement('tr'); //this data is all of the json which you get in a response 
        //to GET request on postman
