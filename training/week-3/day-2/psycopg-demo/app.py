...
# to use the values call conn(port=config['port'], host=config['host'], ...)
# Make sure you're using the password for your database whenever you installed Postgres onto your machine
conn = psycopg.connect(host=config['host'], port=config['port'], dbname=config['dbname'], user=config['user'],
...
cur.execute(sql_query_template, ('testing145', '512-826-0008'))
